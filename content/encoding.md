---
title: "Encoding"
date: 2021-03-18T21:57:33+08:00
draft: true
---
...
推荐阅读 [很多网站源码都是分为 GBK 和 UTF-8 版，为什么要同时开发两种](https://www.zhihu.com/answer/676684648)
...
|Char |Askii|Env    |Meaning
|:----|:----|:------|:-----
|LF   |\n   |*nix   |换行
|CR   |\r   |mac    |回车
|CRLF |\r\n |windows|回车换行

所以往往会产生很多编码的问题, 如 Git 提交的时候在用 CRLF 会出现一些奇怪的 Bug. 又比如: 

- [在 Windows 下键入 Enter 键，在键盘缓冲区中是存入 '\n' 还是 "\r\n"？](https://www.zhihu.com/question/24639606/answer/28476223)

> 从USB的请求开始到最终窗口收到消息，中间有好几层缓冲区，每层都不太一样. USB驱动一层有缓冲区，扫描码，不区分换行符, directX的directIO在这一层就可以直接把按键信息拿走. 之后消息通过内核到用户态, 扫描码 根据区域符号转化 变成按键(\n), 再分发消息给各个窗口、线程等(\n), 部分窗口会根据按键消息做文本转换，转换完以后，看到的才是"\r\n".
> 
> 命令行的C语言，由Windows的POSIX子系统或者其它模块负责接收按键消息，在LibC一层通过stdio/stdlib这些库转化成"\r\n"
